...
const Post = ({ post, card, user, type, isUser }) => {
  ...
  const [selectedValue, setSelectedValue] = isUser
    ? useState(post.quantity)
    : useState(0);
  ...
  useEffect(() => {
    if (isUser && !first) {
      const timer = setTimeout(() => {
        axios
          .put(`http://18.229.90.36:3000/updatesalequantity/${post.id}`, {
  ...
  const handleBid = async () => {
    try {
      const response = await axios.post("http://18.229.90.36:3000/makebid", {
        auctionId: post.id,
        userId: state.userId,
        bidAmount: bidAmount,
      });
  ...
  const handleHighestBid = async () => {
    try {
      const response = await axios.get(
        `http://18.229.90.36:3000/searchauctionwinners/${post.id}`
      );
                  ...
                  <>
                    {type == "Sales" ? (
                      <View className="justify-center flex-col items-center flex-1">
                        ...
                        >
                          <View
                            className="rounded-b-lg flex-1 justify-center"
                    ...
                    >
                      ...
                      >
                        <View className="bg-sky-900 p-2 m-8 rounded-lg items-center max-h-96">
                          <Text className="text-xl font-bold text-slate-100 text-center my-3">
                            Are you sure you want to{" "}
                            {type == "Auctions"
                              ? "put a bid? You cannot undo this"
                              : "buy? It will open a chat with the seller"}
                          </Text>
                          {type == "Auctions" && (
                            <Input
                              label="Bid Amount:"
                          ...
                          <Button
                            ...
                            onPress={() => {
                              if (type == "Sales") {
                                handleWhatsappPress({
                                  message: `Hi ${user.nickname}, I contact you for buying ${post.quantity} of your ${card.name} sale.`,
                                  phone: user.contact,
                                });
                              } else {
                                handleBid();
                    ...
                    {type == "Sales" && (
                      <View className="flex-row justify-between border-2 border-sky-900 rounded-lg items-center flex-1 bg-sky-900 h-full mr-1">
                        ...
                        <Pressable
                          onPress={() =>
                            setSelectedValue(
                              selectedValue === 1 ? 1 : selectedValue - 1
                            )
                          }
                          style={{ flex: 1 }}
                        >
                        ...
                        <View className="flex-row flex-1 mr-1">
                          ...
                          >
                            ...
                            >
                              <View className="bg-sky-900 p-2 m-8 rounded-lg items-center max-h-96">
                                <Text className="text-xl font-bold text-slate-100 text-center my-3">
                                  You are going to contact the highest bidder in
                                  this auction, do you wish to proceed?
                                </Text>
                                <Button
                                  title="Confirm"
                                  loading={false}
                                  ...
                                  onPress={() => {
                                    ...
                                    handleWhatsappPress({
                                      message: `Hi, you were the user with the highest bid in my x${post.quantity} ${card.name} auction, I'm ${user.nickname}`,
                                      phone: highestBid.contact,
                                    });
                                    setIsEndVisible(!isEndVisible);
                                  }}
                                />
                            ...
                            <Button
                              ...
                              onPress={() => {
                                if (type == "Sales") {
                                  ...
                                  deleteSale();
                                  const newData = salesUserData.filter(
                                  ...
                                  deleteAuction();
                                  const newData = auctionsUserData.filter(
                                    (item) => item.post.id !== post.id
                                  );
          ...
          {isVisible && !isUser && (
            <View className="flex-row mt-1">
              <View className="w-24 items-center mt-1">
                ...
                />
                <WhatsAppButton
                  onSubmit={handleWhatsappPress}
                  message={`Hi ${user.nickname}, I contact you for your ${card.name} post`}
                />
              </View>
